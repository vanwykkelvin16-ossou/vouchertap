...
-- A recurring voucher re-opens for every member at the start of each calendar
-- month (Africa/Johannesburg time), even after they've claimed/used it.

-- 1) Flag on vouchers
ALTER TABLE public.vouchers
  ADD COLUMN IF NOT EXISTS is_recurring boolean NOT NULL DEFAULT false;

-- 2) Cycle key on claims: 'once' for normal vouchers, 'YYYY-MM' for recurring
ALTER TABLE public.voucher_claims
  ADD COLUMN IF NOT EXISTS cycle_key text NOT NULL DEFAULT 'once';

-- 3) Replace one-claim-ever uniqueness with one-claim-per-cycle
ALTER TABLE public.voucher_claims
...
-- 4) Claim RPC: recurring vouchers stamp the claim with the current month,
--    so next month the unique index no longer blocks a fresh claim.
